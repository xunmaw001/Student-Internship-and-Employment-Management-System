...
//            toudi.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        ...
        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        ToudiEntity toudiEntity = toudiService.selectOne(queryWrapper);
        if("".equals(toudi.getZhaopinFile()) || "null".equals(toudi.getZhaopinFile())){
                toudi.setZhaopinFile(null);
        }
        if(toudiEntity==null){
...
//                            toudiEntity.setYonghuId(Integer.valueOf(data.get(0)));   //学生 要改的
//                            toudiEntity.setZhaopinId(Integer.valueOf(data.get(0)));   //招聘 要改的
//                            toudiEntity.setZhaopinFile(data.get(0));                    //简历 要改的
