...
public class ZhaopinLiuyanController {
    ...
    */
    ...
    public R update(@RequestBody ZhaopinLiuyanEntity zhaopinLiuyan, HttpServletRequest request){
...
//            zhaopinLiuyan.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        //根据字段查询是否有相同数据
        Wrapper<ZhaopinLiuyanEntity> queryWrapper = new EntityWrapper<ZhaopinLiuyanEntity>()
            .eq("id",0)
            ;

        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        ZhaopinLiuyanEntity zhaopinLiuyanEntity = zhaopinLiuyanService.selectOne(queryWrapper);
        zhaopinLiuyan.setUpdateTime(new Date());
        if(zhaopinLiuyanEntity==null){
